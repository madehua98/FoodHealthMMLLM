...
CONV="phi"
CHECKPOINT_DIR="/media/fast_data/model/checkpoints/checkpoints-phi-2.7b-moe-v2k_0426"
BASE_MODEL_NAME="MoE-LLaVA-Phi2-2.7B-4e-v2k_0426"
...
checkpoints_to_test=("checkpoint-14500" "checkpoint-8500" "checkpoint-12000")
...
for CKPT in "${checkpoints[@]}"; do
  ...
  if [[ " ${checkpoints_to_test[@]} " =~ " ${checkpoint_name} " ]]; then
    ...
    for i in "${!GPUs[@]}"; do
      ...
      question_file=${split_files[$i]}  # Get the corresponding split file for the current GPU
      log_file="${out_path}/log_gpu${gpu}_${checkpoint_name}.txt"
