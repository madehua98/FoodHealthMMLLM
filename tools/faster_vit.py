...
def window_reverse(windows, window_size, H, W, B):
    ...
    # B = (windows.shape[0] * window_size * window_size) // (H * W)
...
class FasterViTLayer(nn.Module):
    ...
    def forward(self, x):
        ct = self.global_tokenizer(x) if (self.global_tokenizer is not None) else None
        # ct = None
        B, C, H, W = x.shape
...
class FasterViT(nn.Module):
    ...
    def __init__(self,
                 ...
                 hat=None,
                 do_propagation=False, mid_output=False, midfeat_len=768,
                 **kwargs):
        """
        Args:
            dim: feature size dimension.
        ...
        self.trans = None
        if not num_features == midfeat_len:
            self.trans = nn.Conv2d(num_features, midfeat_len, 1, bias=False)
            num_features = midfeat_len
...
def faster_vit_1_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(pretrained, map_location='cpu'))
        print('pretrained loaded', pretrained)
    return model
...
if __name__ == "__main__":
    ...
    image_size = 224
    img = torch.ones([2, 3, image_size, image_size]).to(device)
    model = faster_vit_2_224(pretrained='/home/xuzhenbo/Downloads/fastervit_2_224_1k.pth.tar').to(device)
    # model = faster_vit_1_224(pretrained='/home/xuzhenbo/Downloads/fastervit_1_224_1k.pth.tar').to(device)
    out = model(img)
