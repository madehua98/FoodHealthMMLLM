...
def generate(image1, textbox_in, first_run, state, state_, images_tensor):
    ...
    if type(state) is not Conversation:
        state = conv_templates[conv_mode].copy() # Conversation(system="A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions.", roles=('USER', 'ASSISTANT'), messages=[], offset=0, sep_style=<SeparatorStyle.TWO: 2>, sep=' ', sep2='<|endoftext|>', version='phi', skip_next=False)
    ...
    if os.path.exists(image1):
        ...
        # print(tensor.shape)
        images_tensor.append(tensor) # 【torch.Size([3, 336, 336])】
